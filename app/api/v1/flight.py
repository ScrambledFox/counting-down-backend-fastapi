...
from app.schemas.v1.flight import Flight, FlightCreate, FlightNumber, FlightUpdate
...
@router.get("/next", summary="Get Next Most Recent and Active Flight Item")
async def get_next_flight_item(
    service: FlightServiceDep,
):
    flight = await service.get_next_flight()
    if flight is None:
        return {"detail": "No active flights found"}
    return flight
...
@router.put("/{flight_id}", summary="Update Flight")
